...
| 83–84 | “Ezan / Dua Dinle”: multiple adhans by prayer/makam, Mecca adhan, extra adhans, Sela, adhan dua, Lebbeyk, iftar dua. | Build 77 now has a real optional 18-second CC0 Adhan notification sound at exact prayer time plus a Settings test action. `PrayerDuaAudioView` remains separate prayer-recitation content. | **PARTIAL / SOURCE-GATED**. The core prayer-time Adhan is implemented; the broader Sela/makam/dua catalogue still requires separately verified audio rights. |
...
3. **Adhan / Sela audio hub**
   - Core notification Adhan is now implemented with a pinned CC0 asset and license record.
   - Keep it separate from prayer-dua recitations.
   - Any additional Fajr-specific Adhan, Sela or catalogue recording must have clear rights/provenance before shipping.
