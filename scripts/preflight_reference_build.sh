...
# Current expected app version after the v3.48 patch chain.
grep -q 'MARKETING_VERSION="3.48"' scripts/build_unsigned_ipa.sh   || fail "expected MARKETING_VERSION 3.48 not present"
grep -q 'CURRENT_PROJECT_VERSION="53"' scripts/build_unsigned_ipa.sh   || fail "expected build number 53 not present"
